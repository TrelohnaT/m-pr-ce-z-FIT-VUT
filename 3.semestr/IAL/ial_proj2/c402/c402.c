...
**/
	...
	tBTNodePtr *novyUzel = RootPtr;	//přidávaný prvek

	if(*novyUzel != NULL)
	{
		while(true)
		{
			/*novy prvek je větší, pujde tedy do prava*/
			if((*novyUzel)->Cont < Content)
			{
				/*pokud však v pravo něco je*/
				if((*novyUzel)->RPtr != NULL)
				{
					/*pokračujeme dál do prava*/
					novyUzel = &(*novyUzel)->RPtr;
				}
				/*pokud ne, novy uzel vytvoříme*/
				else
				{
					/*novy uzel vytvoříme*/
					novyUzel = &(*novyUzel)->RPtr;
					*novyUzel = malloc(sizeof(struct tBTNode));
					(*novyUzel)->Cont = Content;
					(*novyUzel)->LPtr = NULL;
					(*novyUzel)->RPtr = NULL;
					return;
				}
			}
			/*novy prvek je menší, pujde tedy do leva*/
			else if((*novyUzel)->Cont > Content)
			{
				/*pokud však v pravo něco je*/
				if((*novyUzel)->LPtr != NULL)
				{
					/*pokračujeme dál do leva*/
					novyUzel = &(*novyUzel)->LPtr;
				}
				/*pokud ne*/
				else
				{
			...
			/*novy prvek se shoduje s nějakým už existujícím*/
			else
			{
				/*takže končíme*/
				return;
...
**/
	...
	/*dokud není ukuzatel na uzel NULL*/
	while (ptr != NULL)
	{
		/*uzly si vytiskneme*/
		BTWorkOut(ptr);
		/*dáváme uzly na zásobník*/
		SPushP(Stack, ptr);
		/*a posouváme se na další uzel*/
		ptr = ptr->LPtr;
	}
...
**/
	/*deklarace a inicializace ukazatele na stack*/
	tStackP stack;
	/*deklarace a inicializace utakzatele na uzly*/
	tBTNodePtr nejLevejsi = RootPtr;
	/*inicializace stack*/
	SInitP(&stack);
	/*hledání toho nejlevějšího uzlu*/
	Leftmost_Preorder(nejLevejsi, &stack);
	/*dokud zásobník není prázdny*/
	while(!SEmptyP(&stack))
	{
		/*získání nového uzlu ze zásobníku*/
		nejLevejsi = STopPopP(&stack);
		/*posun a hledání toho nejlevějšího uzlu*/
		Leftmost_Preorder(nejLevejsi->RPtr, &stack);
	}
...
** realizujte jako volání funkce BTWorkOut().
**/
	/*ukazatel na stack ukazatelu na uzly*/
	tStackP stack;
	/*deklarace a inicializace ukazatele na uzel*/
	tBTNodePtr nejLevejsi = RootPtr;
	/*inicializace zásobníku*/
	SInitP(&stack);
	/*hledání toho nejlevejsího*/
	Leftmost_Inorder(nejLevejsi, &stack);
	...
	while(!SEmptyP(&stack))
	{
		...
		nejLevejsi = STopPopP(&stack);
		/*tisk uzlu*/
		BTWorkOut(nejLevejsi);
		/*posun a hledání hového nejlevejsího*/
		Leftmost_Inorder(nejLevejsi->RPtr, &stack);
	}
...
**/
	...
	while (ptr != NULL)
	{
		/*dáváme uzly na zásobník*/
		SPushP(StackP, ptr);
		/*dáváme bool hodnotu do bool zasobníku*/
		SPushB(StackB, true);
		/*a posouváme se na další uzel*/
		ptr = ptr->LPtr;
	}
...
**/
	/*ukazatel na stack ukazatelu na uzly*/
	tStackP stackP;
	/*ukazatel na stack bool hodnot*/
	tStackB stackB;
	/*deklarace a inicializace ukazatele*/
	tBTNodePtr nejLevejsi = RootPtr;
	/*deklarace bool promněné*/
	bool hodnotaBool;
	/*inicializace stack ukazatelu*/
	SInitP(&stackP);
	/*inicializace stack bool hodnot*/
	SInitP(&stackB);
	/*hledání toho nejlevějšího uzlu*/
	Leftmost_Postorder(nejLevejsi, &stackP, &stackB);
	/*dokud zásobník není prázdny*/
	while(!SEmptyP(&stackP))
	{
		/*získání nového uzlu ze zásobníku*/
		nejLevejsi = STopPopP(&stackP);
		/*získání nové bool hodnoty ze zásobníku*/
		hodnotaBool = STopPopB(&stackB);
		/*pokud je hodnota true jde se ještě do leva*/
		if(hodnotaBool)
		{
			/*uložení dalšího uzlu na zásobník*/
			SPushP(&stackP, nejLevejsi);
			/*uložení hodnoty false na zásobník*/
			SPushB(&stackB, false);
			/*hledání nejlevějšího a posun do prava*/
			Leftmost_Postorder(nejLevejsi->RPtr, &stackP, &stackB);
		}
		/*pokud je hodnota false*/
		else
		{
			/*uzel vytiskneme*/
			BTWorkOut(nejLevejsi);
...
**/
	...
	tBTNodePtr mazanyPrvek = *RootPtr;

	tStackP stack;

	SInitP(&stack);
	Leftmost_Inorder(mazanyPrvek, &stack);

	while(!SEmptyP(&stack))
	{
		mazanyPrvek = STopPopP(&stack);
		Leftmost_Inorder(mazanyPrvek->RPtr, &stack);
		free(mazanyPrvek);
	}
