...
*/
	/*Kontrola parametru s*/
	if(s == NULL)
	{
		stackError(SERR_INIT); //volání funkce pro chybu
	}
	else
	{
		s->top = -1; //zásobník je inicilizován a vrchol jen na pozici -1
	}
}
...
*/
	return (s->top == -1); //kontrola zda je zásobník prázdný
}
...
*/
	return (s->top >= (STACK_SIZE - 1)); //kontrola, zda zásobník nepřesáhnul 20 prvků (velikost)
}
...
*/
	...
	/*ověření, zda zásobník není prázdný*/
	if(stackEmpty(s) == 0)
	{
		*c = s->arr[s->top];	//není, takže je do parametru c nahrán prvek z vrcholu
	}
	else
	{
		stackError(SERR_TOP);	//je, takže je zavolána chybová funkce
	}
}
...
** 
*/
	/*Kontrola, zda je zásobník neprázdný*/
	if(stackEmpty(s) == 0)
	{
		if(s->top > -1) //zabezpečení, aby vrchol zásobníku nebyl menší než -1 při zavolání nad prázdným zásobníkem
		{
			s->top--;	//odstranění prvku na vrcholu
		}
	}
}
...
*/
	...
	/*ověření, zda je zásobník plný*/
	if(stackFull(s) == 0)
	{
		/*není, takže se muže vložit další prvek*/
		s->top++;	//posunutí vrcholu zásobníku
		s->arr[s->top] = c;		//vpolžení obsahu parametru c do nového prvku na vrcholu
	}
	else
	{
		stackError(SERR_PUSH);	//chybová funkce
	}
}
/*S gramatikou mojich komentářu jsem se moc neobtěžoval, protože to stejně nikdo číst nebude. Děkuji za pochopení*/
/* Konec c202.c */
